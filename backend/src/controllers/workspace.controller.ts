...
import {
  ...
  getPendingWorkspaceMembersService,
...
export const getPendingWorkspaceMembersController = asyncHandler(
  async (req: Request, res: Response) => {
    const workspaceId = workspaceIdSchema.parse(req.params.id);
    const userId = req.user?._id;

    const { members } = await getPendingWorkspaceMembersService(workspaceId);
