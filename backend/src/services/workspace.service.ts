...
import { ensureWorkspaceGroupChat } from "./chat.service";
...
export const createWorkspaceService = async (
  ...
  // Auto-create workspace-wide group chat
  try {
    await ensureWorkspaceGroupChat(
      (workspace._id as mongoose.Types.ObjectId).toString(),
      (user._id as mongoose.Types.ObjectId).toString()
    );
  } catch (e) {
    console.error("Could not create workspace group chat:", e);
  }
...
export const getPendingWorkspaceMembersService = async (workspaceId: string) => {
  const members = await PendingMemberModel.find({
    workspaceId,
    status: "PENDING",
  }).populate("userId", "name email profilePicture -password");
...
export const getWorkspaceAnalyticsService = async (workspaceId: string, userId: string) => {
...
export const getWorkspaceAnalyticsServiceAdmin = async (workspaceId: string) => {
  const currentDate = new Date();
